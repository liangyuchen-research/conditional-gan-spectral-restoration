...
def run():
    ...
    def ask_csv(title):
        ...
        path = filedialog.askopenfilename(
            title=title, filetypes=[("CSV files","*.csv")]
        )
        if not path:
    ...
    def main():
        ...
        old_path = ask_csv("Select the original standard CSV (wavelength header).")
        new_path = ask_csv("Select the new standard CSV (wavelength header).")
        unknown_path = ask_csv("Select the unknown-sample CSV (wavelength header).")
        ...
        if old_std.shape[1] != new_std.shape[1] or unk.shape[1] != new_std.shape[1]:
            raise ValueError("Original, new, and unknown spectra have inconsistent wavelength counts.")
        global L
        L = new_std.shape[1]
        ...
        out_full_str = unk_full_str.copy()
        ...
        plt.plot(wl, new_std_aligned[idx, :], '--', label=f'New standard (5 ppm, after alignment)', linewidth=2)
        plt.plot(wl, cal_new_std_for_plot[idx, :], ':', label='Corrected new standard (5 ppm)', linewidth=2)
